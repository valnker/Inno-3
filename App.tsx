      ...
      for (const story of storiesToUpdate) {
        try {
          ...
          try {
            ...
            // Update state immediately to show progress on screen
            setStories(prev => prev.map(s => s.id === story.id ? {...s, coverImage: imageUrl} : s));
          } catch (e) {
            console.error('Could not save cover to localStorage. Storage might be full.', e);
            if (!storageAlertShown) {
              alert("Oh no! Could not save a story cover because your browser's storage is full. Some images might not be saved correctly on your next visit.");
              storageAlertShown = true;
            }
            // Even if storage fails, update state for the current session
            setStories(prev => prev.map(s => s.id === story.id ? {...s, coverImage: imageUrl} : s));
          }
        } catch (error) {
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            ...
            >
                     ...
                     <div className="absolute top-2 right-2 flex flex-col gap-2 transition-opacity duration-300 opacity-0 group-hover:opacity-100">
                        ...
                        {story.id >= 1000 && (
                          <button
                            ...
                            className="p-2 bg-white/80 backdrop-blur-sm rounded-full text-gray-700 hover:bg-white hover:text-red-500 transition-all"
                ...
                <div className="p-4 flex-grow flex flex-col justify-between">
                    <div>
                        <h2 className="text-xl font-bold text-gray-800 truncate group-hover:text-amber-600 transition-colors">{story.title}</h2>
                        <span className={`inline-block mt-2 px-3 py-1 text-sm font-semibold rounded-full ${getLevelClasses(story.level)}`}>
                            {story.level}
                        </span>
                    </div>
                </div>
            ...
            <div 
                className="group bg-white/70 backdrop-blur-sm border-4 border-dashed border-sky-300 rounded-2xl shadow-lg cursor-pointer transform hover:-translate-y-2 transition-transform duration-300 overflow-hidden flex flex-col items-center justify-center text-center p-4 min-h-[240px] hover:border-amber-400 hover:bg-white"
                onClick={() => setIsCreateModalOpen(true)}
                aria-label="Create a new story"
                role="button"
            >
                <div className="text-sky-500 group-hover:text-amber-500 transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                    </svg>
                </div>
                <p className="mt-2 text-xl font-bold text-sky-700 group-hover:text-amber-700 transition-colors">Create a New Story</p>
            </div>
        </div>
      </main>
    </div>
